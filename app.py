...
# --- 🆕 公開設定の読み書き機能 ---
def get_status():
    """現在の募集ステータスを取得（True=募集中, False=停止中）"""
    try:
...
with tab3:
    ...
    if st.session_state['login_attempts'] >= 5:
        ...
        if password:
            # 🛡️ try-except で英語のエラーコードが出るのを防ぐ
            try:
                # パスワード正解チェック
                # .get()を使うことで、もし設定ファイルにキーがなくてもクラッシュさせない
                correct_pass = st.secrets.get("ADMIN_PASSWORD")
                
                if not correct_pass:
                    st.warning("⚠️ システム設定エラー: パスワードが設定されていません。管理者に連絡してください。")
                    ...
                    if st.button("🚀 自動マッチングを実行"):
                        if df_students.empty or df_mentors.empty:
                            st.warning("データが不足しています。") # ここもerrorからwarningへ
                        else:
                            # --- マッチングロジック ---
                            results = []
                            mentor_schedule = {} 
                    ...
                    time.sleep(3) # 3秒待機（攻撃対策）
                    
                    # 🔴 ここがポイント：st.errorではなくst.warning（黄色）にする
                ...
                # 万が一、予期せぬエラー（Secretsが無いなど）が起きてもここでキャッチ
